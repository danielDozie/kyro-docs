...
# ==============================================================================
# Kyro CMS — Netlify Zero-Touch Deployment Script (PostgreSQL + S3/R2)
# Public Endpoint: https://<docs-domain>/scripts/deploy-netlify.sh
# Template : Blog (posts, pages, categories, media, menu, users, audit_logs, forms)
# Database : Auto-Provisioned Neon PostgreSQL (or external PostgreSQL via DATABASE_URL)
# Storage  : S3 / Cloudflare R2 / Netlify Blobs
# Hosting  : Netlify Edge & Serverless
# User Seed: Automatic First Super Admin Bootstrapping
# ==============================================================================

echo "=============================================================================="
echo "🚀 Kyro CMS Netlify Zero-Touch Deployment"
echo "=============================================================================="
...
# 2. Check Package Manager & Netlify CLI
...
if ! npx netlify status >/dev/null 2>&1; then
    echo "🔑 Authenticating with Netlify CLI..."
    npx netlify login
fi
...
echo "⚙️ Generating netlify.toml..."
cat <<EOF > netlify.toml
[build]
  command = "$PACKAGER run build"
  publish = "./admin/dist"
...
# 7. Build Workspace & Deploy to Netlify
echo "🛠️ Compiling workspace & deploying to Netlify..."
$PACKAGER run build
DEPLOYMENT_URL=$(npx netlify deploy --prod --dir="./admin/dist" --json 2>/dev/null | grep -o '"deploy_url":"[^"]*"' | head -n 1 | cut -d'"' -f4 || true)

echo ""
echo "=============================================================================="
echo "🎉 Zero-Touch Kyro CMS Netlify Deployment Complete!"
echo "=============================================================================="
echo "• Deployment URL : ${DEPLOYMENT_URL:-https://$PROJECT_NAME.netlify.app}"
echo "• Admin Dashboard: ${DEPLOYMENT_URL:-https://$PROJECT_NAME.netlify.app}/admin"
echo "• Template       : Blog"
echo ""
